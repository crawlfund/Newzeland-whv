var FamilyName = 'Joey'; //name
var GivenName = 'Qiang'; //name
var Gender = 1; //Male = 1 Female = 2
var Year = 1992; //年月日的年
var Month = 2; //年月日的月
var Day = 13; //年月日的日，输入数字
var Country = 44; //出生地。中国是44，不需要修改
//下面填当前的联系地址
var StreetName = 'xx' //街道地址，填英文地址，替换xx,注意保留引号
var City = 'TianJin' //城市，请修改引号内的内容，保留引号!
var CountryCurrent = 44 //此处44代表中国，不需要修改
var EmailAdress = 'xxx@qq.com' //电子邮件地址
//个人信息第二页
var PassportNumber = 'E09669411' //此处填你的护照号
//下面是护照有效期限
var PassportYear = 2024; //年月日的年
var PassportMonth = 8; //年月日的月
...
var IssuedYear = 2016; //年月日的年
var IssuedMonth = 3; //年月日的月
var IssuedDay = 10; //年月日的日，输入数字
...
var Funds = 2; //无1，有2
var ExpectYear = 2; //预计登录年份，2016填1,2017填2
var ExpectMonth = 5; //预计登录月份
...
var whv_get = function() {
		...
		if (address == 'https://onlineservices.immigration.govt.nz/WorkingHoliday/') {
			...
			if (document.getElementById('ctl00_ContentPlaceHolder1_identification_passportNumberTextBox').value == 0) fill_personal2();
			else window.close();
			...
			if (document.getElementById('ctl00_ContentPlaceHolder1_medicalConditions_renalDialysisDropDownList').selectedIndex == 0) fill_Medical1();
			else window.close();
			...
			if (document.getElementById('ctl00_ContentPlaceHolder1_character_imprisonment5YearsDropDownList').selectedIndex == 0) fill_Character();
			else window.close();
			...
			if (document.getElementById('ctl00_ContentPlaceHolder1_offshoreDetails_commonWHSQuestions_previousWhsPermitVisaDropDownList').selectedIndex == 0) fill_WorkingHoliday()
		}
	}
